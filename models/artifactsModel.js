...
class Artifact {
    constructor(id, name, current_owner, current_position, drop_user) {
        this.id = id;
        this.name = name;
    ...
    static async getArtifactsInBoard(game) {
        try {
            ...
            for (let art of artifacts) {
                result.push(new Artifact(art.ga_id, art.art_name, art.ga_current_owner, art.ga_current_position, art.ga_drop_user));
            }
    ...
    static async getCollectedArtifacts(game) {
        try {
            ...
            for (let art of artifacts) {
                result.playerArtifacts.push(new Artifact(art.ga_id, art.art_name, art.ga_current_owner, art.ga_current_position, art.ga_drop_user));
            }
            ...
            for (let art of artifacts) {
                result.oppArtifacts.push(new Artifact(art.ga_id, art.art_name, art.ga_current_owner, art.ga_current_position, art.ga_drop_user));
            }
            return { status: 200, result: result }
        } catch (err) {
            console.log(err);
