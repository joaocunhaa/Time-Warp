...
class Artifact{
    ...
    static async getArtifactsInBoard(game){
        try{
            let result = [];
            let [artifacts] = await pool.query('select * from game_artifact, artifact where ga_art_id = art_id and ga_gm_id = ?', [game.id]);
            for(let art of artifacts){
                result.push(new Artifact(art.ga_id, art.art_name, art.ga_current_owner, art.ga_current_owner));
            }
